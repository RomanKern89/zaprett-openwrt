...
|---|---|---|
| **Версия** | 1.1.0-r1 | 0.1.4 |
| **Для чего** | все устройства в доме: телефоны, телевизоры, приставки, консоли | один ПК с Windows 10/11 |
| **Установка** | бандл `.tar.gz` и `sh install.sh` | `zaprett-0.1.4-x64-setup.exe` (или MSI для администраторов) |
...
Нет роутера с OpenWrt или обход нужен только на одном компьютере? **zaprett для Windows 0.1.4** делает то же на ПК с
Windows 10 (2004 и новее, включая LTSC 2021) и Windows 11 x64:

- мастер первой настройки: выбор сервисов, поиск мешающих программ, запуск и проверка сайтов;
- проверка сайтов по кнопке и монитор доступности по расписанию с предупреждением и автоматическим восстановлением;
- автоподбор стратегии (быстрый и полный) без остановки обхода, свои стратегии с проверкой движком;
- диагностика «Чем блокирует провайдер», поиск конфликтующих программ, диагностический отчёт;
- свои списки сайтов и IP-сетей, подписки на внешние списки;
- значок в области уведомлений: включить и выключить обход, проверить сайты, закрыть zaprett полностью;
- интерфейс на русском, английском и китайском;
- менять настройки могут администраторы и группа «zaprett Operators», остальным — режим «Только просмотр»;
- тихая установка с параметрами для администраторов.

Один установщик — среда .NET, драйвер WinDivert и движки zapret и zapret2 уже внутри. Скачивайте
`zaprett-0.1.4-x64-setup.exe`: он сразу спрашивает права администратора и запускает установку; сам `.msi` — для
...
1. Откройте выпуск [**zaprett для Windows 0.1.4**](https://github.com/RomanKern89/zaprett-openwrt/releases/tag/win-v0.1.4)
   и скачайте `zaprett-0.1.4-x64-setup.exe` и `SHA256SUMS`.
2. Проверьте файл в PowerShell: `Get-FileHash .\zaprett-0.1.4-x64-setup.exe -Algorithm SHA256` — значение должно
   совпасть со строкой в `SHA256SUMS`.
3. Запустите файл. Установщик пока не подписан, поэтому в окне «Система Windows защитила ваш компьютер» нажмите
   **«Подробнее» → «Выполнить в любом случае»**, а в окне «Разрешить этому приложению вносить изменения?» — **«Да»**.
4. Пройдите страницы установщика (значок при входе и ярлык на рабочем столе включены по умолчанию) и нажмите «Готово» —
   откроется мастер: выберите сервисы, и zaprett сам подберёт работающую стратегию.

Обновление — тот же `setup.exe` поверх установленной версии, настройки сохраняются. Тихая установка и развёртывание —
файл `.msi` ([раздел 16](windows/README.md#16-для-администраторов-тихая-установка)). Подробно со снимками экрана —
[разделы 4–6 руководства](windows/README.md#4-скачивание-и-проверка-файла).
