...
for series in 25.12 24.10; do
	...
	run_case install "$series" 0 "$PREP; tar -xzf /b/$bn.tar.gz; cd $bn; sh install.sh; $CHECK; /usr/libexec/zaprett/nfqws --version; ls -l /usr/libexec/zaprett/nfqws; sh install.sh; $CHECK; sh install.sh --uninstall; $GONE; sh install.sh --uninstall --purge; $KEYGONE; if [ -e /etc/config/zaprett ]; then echo config left; exit 1; fi"
