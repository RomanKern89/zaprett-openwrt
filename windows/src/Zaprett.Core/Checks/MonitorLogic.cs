...
public static class MonitorLogic
{
    ...
    public static readonly IReadOnlyList<string> SkipReasons = ["monitor_disabled", "service_disabled", "stopped", "not_running", "job_busy"];

    public static JsonObject Empty() => new()
    {
        ["state"] = "unknown", ["checked_at"] = null, ["ok"] = 0, ["total"] = 0, ["consecutive_failures"] = 0,
        ["history"] = new JsonArray(), ["last_repair"] = null,
    };

    /// <summary>The state after one check. A check fails when fewer than half of the targets answered (ok * 2 &lt; total);
    /// total 0 does not count (state unknown). repair: degraded, auto_repair on and no repair within 6 hours.</summary>
    public static (JsonObject State, bool Repair) Step(JsonObject? prev, int ok, int total, long now, MonitorConfig m)
    {
        ...
        history.Add(new JsonObject { ["t"] = now, ["ok"] = ok, ["total"] = total });
        while (history.Count > History)
            history.RemoveAt(0);
        var fails = ok * 2 < total ? (R.Long(st["consecutive_failures"]) ?? 0) + 1 : 0;
        st["consecutive_failures"] = fails;
        st["state"] = fails >= m.Threshold ? "degraded" : "ok";
        var last = R.Long(st["last_repair"]?["t"]);
        var repair = R.Str(st["state"]) == "degraded" && m.AutoRepair && (last == null || now - last >= RepairInterval);
        return (st, repair);
    }

    /// <summary>Why the monitor does not check now, or null.</summary>
    public static string? SkipReason(ZaprettConfig cfg, bool stopped, bool running, bool busy)
    {
